...
export function RecruitmentManagePage({
  ...
  function handleSaveQuestion() {
    ...
    if (qToSave.type === "TABLE") {
      ...
      if (!qToSave.limit_length) {
        qToSave.limit_length = 800;
      }
    }
        ...
        <div className="space-y-5">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                공통 문항 및 분석/엔지니어링/시각화 트랙별 전용 문항과 테이블 형태 문항(행/열
                메타데이터)을 설정합니다.
              </p>
            </div>
            <button
          ...
          <div className="space-y-3">
            {filteredQuestions.map((q) => {
              ...
              return (
                <div
                  key={q.id}
                  className="rounded-2xl border border-slate-200 bg-white p-5 space-y-3.5 shadow-[0_1px_3px_rgba(0,0,0,0.03)] hover:border-slate-300 transition-all"
                >
                  <div className="flex items-start justify-between flex-wrap gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      {q.has_answers && (
                        <span
                          className="text-[10px] px-1.5 py-0.2 rounded font-bold bg-amber-50 text-amber-700 border border-amber-200"
                          title="제출된 답변이 있어 삭제 불가"
                        >
                          답변 작성됨
                        </span>
                      )}
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1 flex-wrap">
                            {q.metadata.columns?.map((col, idx) => (
                              <span
                                key={idx}
                                className="px-2 py-0.5 rounded bg-white text-slate-700 border border-slate-200 font-mono text-[11px] shadow-2xs"
                              >
                                {col}
                              </span>
                            ))}
          ...
          <div className="rounded-2xl bg-white border border-slate-200 p-8 shadow-sm space-y-6">
            <div className="border-b border-slate-200 pb-4 space-y-1">
              ...
              <p className="text-xs text-slate-500">
                작성 중인 내용은 임시저장(DRAFT)되며, 최종 제출(SUBMITTED) 전까지 자유롭게 수정할 수
                있습니다.
              </p>
            </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                ...
                <div>
                  ...
                  <span className="font-bold text-blue-600">
                    {CATEGORY_META[previewTrack].label}
                  </span>
                </div>
                <div>
            ...
            <div className="space-y-6">
              <h3 className="text-xs font-bold text-slate-700">
                2. 자기소개 및 전공 역량 문항 ({previewQuestions.length}문항)
              </h3>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        {q.is_required && <span className="text-red-500 font-bold text-xs">*</span>}
                      </div>
                      {q.limit_length && (
                        <span className="text-xs text-slate-400 font-mono">
                          0 / {q.limit_length}자
                        ...
                        <table className="w-full text-xs border border-slate-200 rounded-lg overflow-hidden bg-white">
                          <thead>
                            <tr className="bg-slate-100 text-slate-700 font-semibold">
                              <th className="text-left px-3 py-2.5 border-r border-slate-200 w-44 bg-slate-200/60">
                                구분 / 항목
                              </th>
                              {q.metadata?.columns?.map((c, i) => (
                                <th
                                  key={i}
                                  className="text-left px-3 py-2.5 border-r border-slate-200 last:border-r-0"
                                >
                                  {c}
                                </th>
                              ))}
      ...
      {activeTab === "csv" && (
        <div className="space-y-5">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              기수 및 트랙별 지원서 데이터를 UTF-8 BOM 엑셀 호환 CSV 형식으로 브라우저에서 즉시
              다운로드합니다.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {(["ANALYSIS", "ENGINEERING", "VISUALIZATION"] as TrackType[]).map((track) => {
              ...
              return (
                <div
                  key={track}
                  className="p-6 rounded-2xl border border-slate-200 bg-white shadow-xs space-y-4 hover:border-slate-300 transition-all"
                >
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-500 font-mono">
                      제{csvTerm}기 지원자: {applicants.length}명
                    </p>
                  </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                모집 오픈 전 알림을 신청한 예비 지원자 명단을 확인하고, 맞춤 발송 문구를 원클릭으로
                복사하여 안내를 발송합니다.
              </p>
            </div>
              ...
              <div className="flex items-center justify-between flex-wrap gap-2 border-b border-slate-100 pb-3">
                <div className="space-y-0.5">
                  ...
                  <p className="text-[11px] text-slate-500">
                    아래 템플릿의 문구를 자유롭게 수정하세요. 실시간으로 오른쪽 미리보기에
                    반영됩니다.
                  </p>
                </div>
              ...
              <table className="w-full text-xs min-w-[760px]">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/40 text-slate-400 font-semibold text-[11px] uppercase tracking-wider whitespace-nowrap">
                    ...
                    <th className="text-center px-4 py-3.5 w-36 text-blue-600 font-bold">
                      맞춤 알림 발송
                    </th>
          ...
          <div className="w-full max-w-md rounded-2xl p-6 bg-white border border-slate-200 shadow-2xl space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewLead((prev) => ({
                      ...prev,
                      interestedTrack: e.target.value as TrackType,
                    }))
                  }
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
              ...
              <button
                ...
                className="px-4 py-1.5 rounded-lg text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 cursor-pointer shadow-xs"
              >
                신청자 추가
              </button>
            </div>
          ...
          <div className="w-full max-w-lg rounded-2xl p-6 bg-white border border-slate-200 shadow-2xl space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
              ...
              <button
                ...
                className="px-4 py-1.5 rounded-lg text-xs font-bold text-white bg-red-600 hover:bg-red-700 cursor-pointer shadow-xs"
              >
                저장하기
              </button>
            </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <label className="text-slate-700 font-bold text-xs flex items-center gap-1">
                        ...
                        <span>
                          열 헤더 목록 (Columns, {editingQuestion.metadata?.columns?.length || 0}개)
                        </span>
                      </label>
                      <button
                        type="button"
                      ...
                      {editingQuestion.metadata?.columns?.map((col, cIdx) => (
                        <div key={cIdx} className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            onClick={() => {
                              const newCols =
                                editingQuestion.metadata?.columns?.filter((_, i) => i !== cIdx) ||
                                [];
                              setEditingQuestion((prev) =>
                                prev
                                  ? {
                  ...
                  <div className="space-y-2 pt-2 border-t border-slate-200">
                    <div className="flex items-center justify-between">
                      <label className="text-slate-700 font-bold text-xs flex items-center gap-1">
                        ...
                        <span>
                          행 항목 목록 (Rows, {editingQuestion.metadata?.rows?.length || 0}개)
                        </span>
                      </label>
                      <button
                        type="button"
                      ...
                      {editingQuestion.metadata?.rows?.map((rowItem, rIdx) => (
                        <div key={rIdx} className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            onClick={() => {
                              const newRows =
                                editingQuestion.metadata?.rows?.filter((_, i) => i !== rIdx) || [];
                              setEditingQuestion((prev) =>
                                prev
                                  ? {
                                      ...prev,
                  ...
                  <div className="pt-2 border-t border-slate-200 space-y-1.5">
                    ...
                    <div className="overflow-x-auto rounded-lg border border-slate-200 bg-white">
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="bg-slate-100 text-slate-700 font-semibold">
                            <th className="px-2.5 py-2 border-r border-slate-200 text-left bg-slate-200/50">
                              행 항목
                            </th>
                            {editingQuestion.metadata?.columns?.map((c, i) => (
                              <th
                                key={i}
                                className="px-2.5 py-2 border-r border-slate-200 last:border-r-0 text-left font-mono text-[11px]"
                              >
                                {c}
                              </th>
                            ))}
                        ...
                        <tbody className="divide-y divide-slate-100 font-mono text-[11px]">
                          {editingQuestion.metadata?.rows?.map((r, rI) => (
                            <tr key={rI}>
                              <td className="px-2.5 py-1.5 border-r border-slate-200 bg-slate-50 font-sans font-bold text-slate-800">
                                {r}
                              </td>
                              {editingQuestion.metadata?.columns?.map((_, cI) => (
                                <td
                                  key={cI}
                                  className="px-2.5 py-1.5 border-r border-slate-200 last:border-r-0 text-slate-400 italic"
                                >
                                  [지원자 입력칸]
                                </td>
                              ))}
