...
import {
  Shield, UserCheck, Key, UserPlus, Lock, Unlock, Trash2, Edit3,
  RefreshCw, CheckCircle2, AlertCircle, Search, Filter, Copy,
  Eye, EyeOff, Check, X, ShieldAlert, Sparkles, Building, User,
  Calendar, Layers, Clock, Award, ShieldCheck, FileJson, CheckSquare,
  Users, UserCog, KeyRound, History, GraduationCap, Laptop,
  Send, ExternalLink, ShieldQuestion, ToggleLeft, ToggleRight,
  Database, FileCode, CheckCheck, Sliders, Terminal, AlertTriangle
} from "lucide-react";
...
export interface SecurityAuditLog {
  ...
  actor: string;
  target: string;
  actionType: "ACCOUNT_CREATE" | "ROLE_CHANGE" | "ACCOUNT_DELETE" | "PASSWORD_RESET" | "PERMISSION_GRANT";
  description: string;
  ipAddress: string;
}

const TEAM_NAMES: AdminTeamName[] = [
  "대표진", "차기대표진", "서비스운영팀", "운영지원팀", "기획팀", "대외협력팀", "디자인팀", "자료연구팀"
];
...
const INITIAL_AUDIT_LOGS: SecurityAuditLog[] = [
  { id: "l1", timestamp: "2026-08-16 19:25:10", actor: "남민서 (MASTER)", target: "김대현 (boaz_ops_lead)", actionType: "PERMISSION_GRANT", description: "운영지원팀 출결 관리 및 HOST 계정 발급 권한 확인", ipAddress: "121.134.82.11" },
  { id: "l2", timestamp: "2026-08-16 18:40:00", actor: "남민서 (MASTER)", target: "신재원 (boaz_design_lead)", actionType: "PASSWORD_RESET", description: "디자인팀장 계정 비밀번호 초기화 및 RefreshToken 무효화", ipAddress: "121.134.82.11" },
  { id: "l3", timestamp: "2026-07-18 13:00:00", actor: "남민서 (MASTER)", target: "신재원 (boaz_design_lead)", actionType: "ACCOUNT_CREATE", description: "신규 운영진 계정 발급 (POST /api/v1/admin/accounts)", ipAddress: "121.134.82.11" },
  { id: "l4", timestamp: "2026-07-10 11:20:00", actor: "남민서 (MASTER)", target: "김대현 (boaz_ops_lead)", actionType: "ACCOUNT_CREATE", description: "운영지원팀장 계정 발급", ipAddress: "121.134.82.11" },
];

const PERMISSION_MATRIX = [
  { name: "ACCOUNT_MANAGE", desc: "운영진 계정 생성·삭제·권한 부여", master: "허용", super: "제한", serviceTeam: "제한", opsTeam: "제한", otherTeam: "제한", host: "제한" },
  { name: "CONTENT_MANAGE", desc: "아카이빙·FAQ·후기·커리큘럼 CUD", master: "허용", super: "허용", serviceTeam: "허용", opsTeam: "제한", otherTeam: "제한", host: "제한" },
  { name: "RECRUITMENT_MANAGE", desc: "모집 공고·질문·지원서 CSV 추출", master: "허용", super: "허용", serviceTeam: "허용", opsTeam: "제한", otherTeam: "제한", host: "제한" },
  { name: "ATTENDANCE_MANAGE", desc: "활동·팀·명단·HOST 계정 발급", master: "허용", super: "허용", serviceTeam: "제한", opsTeam: "🌟 O (전담)", otherTeam: "제한", host: "제한" },
  { name: "ATTENDANCE_APPROVE", desc: "제출 후 출결 수동 수정·사유 인정", master: "허용", super: "허용", serviceTeam: "제한", opsTeam: "🌟 O (전담)", otherTeam: "제한", host: "제한" },
  { name: "SCORE_MANAGE", desc: "활동 점수 수동 조정 및 점수 확정", master: "허용", super: "허용", serviceTeam: "제한", opsTeam: "🌟 O (전담)", otherTeam: "제한", host: "제한" },
  { name: "RULE_EDIT / ACTIVATE", desc: "점수 규칙 DRAFT 작성 및 활성화", master: "허용", super: "대표진 전용", serviceTeam: "제한", opsTeam: "제한", otherTeam: "제한", host: "제한" },
  { name: "EVALUATION", desc: "서류 평가 (차기: 전 부문 / 일반: 본인 트랙)", master: "본인", super: "전 부문", serviceTeam: "본인", opsTeam: "본인", otherTeam: "본인", host: "제한" },
  { name: "FINAL_DECISION", desc: "최종 합불 판정 및 확정", master: "허용", super: "대표진 전용", serviceTeam: "제한", opsTeam: "제한", otherTeam: "제한", host: "제한" },
];
...
export function SystemAccountsPage({ initialSubTab = "accounts" }: SystemAccountsPageProps) {
  ...
  const [pwTargetAccount, setPwTargetAccount] = useState<{ id: number; username: string; name: string; isSelf: boolean } | null>(null);
  ...
  function generateRandomPassword() {
    let gen = "Boaz!" + Math.floor(1000 + Math.random() * 9000);
    setNewPassword(gen);
  ...
  const filteredAccounts = useMemo(() => {
    return accounts.filter(acc => {
      ...
      if (teamFilter !== "ALL" && acc.team_name !== teamFilter) return false;
      if (trackFilter !== "ALL" && acc.track !== trackFilter) return false;
      if (searchQuery.trim()) {
  ...
  function handleCreateAccount(e: React.FormEvent) {
    ...
    if (!newUsername.trim()) { alert("아이디(username)를 입력해 주세요."); return; }
    if (accounts.some(a => a.username === newUsername.trim())) {
      alert("DUPLICATE_USERNAME: 이미 존재하는 관리자 아이디입니다.");
      return;
    }
    if (!validatePassword(newPassword)) {
      alert("비밀번호 정책 위반: 8자 이상 + 영문, 숫자, 특수문자(!@#$%^&*)를 각 1개 이상 포함해야 합니다.");
      return;
    }
    if (!newName.trim()) { alert("이름을 입력해 주세요."); return; }
    if (newTerm < 0) { alert("기수는 0 이상이어야 합니다."); return; }
  ...
  function handleSaveEdit() {
    ...
    setAccounts(prev =>
      prev.map(a => (a.id === editingAccount.id ? {
        ...a,
        name: editName.trim(),
        role: editRole,
        track: editTrack,
  ...
  function handleDeleteAccount(acc: AdminAccountDto) {
    ...
    if (confirm(`정말 운영진 계정 '${acc.username} (${acc.name})'을(를) 삭제하시겠습니까?\n(DELETE /api/v1/admin/accounts/${acc.id})`)) {
  ...
  function handleSavePassword() {
    ...
    if (!validatePassword(newPasswordInput)) {
      alert("새 비밀번호 정책 위반: 8자 이상 + 영문, 숫자, 특수문자(!@#$%^&*)를 각 1개 이상 포함해야 합니다.");
      return;
    }
    ...
    setAuditLogs(prev => [
      {
        ...
        description: pwTargetAccount.isSelf ? "본인 비밀번호 변경" : "MASTER 권한으로 비밀번호 강제 초기화",
        ipAddress: "121.134.82.11",
      },
      ...prev,
    ]);

    alert(`✅ [PATCH /api/v1/admin/accounts/${pwTargetAccount.id}/password]\n비밀번호가 성공적으로 변경되었습니다.\n해당 계정의 Refresh Token이 무효화되어 재로그인이 요구됩니다.`);
    ...
    <div className="space-y-5" style={{ fontFamily: "'Pretendard Variable', Pretendard, -apple-system, sans-serif" }}>
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-3 flex-wrap gap-3">
        <div className="flex items-center gap-6">
          {[
            { id: "accounts", label: isMaster ? "운영진 계정 관리 (MASTER 전용)" : "내 계정 정보 및 보안", icon: UserCog, count: isMaster ? accounts.length : 1 },
            { id: "permissions", label: "권한 매트릭스 (10대 Permission)", icon: ShieldCheck, count: "Matrix" },
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="p-4 rounded-xl border border-purple-500/30 bg-white space-y-1">
                  ...
                  <p className="text-xl font-bold text-purple-400 font-mono">{stats.superCount}명</p>
                  <p className="text-[10px] text-purple-400/70">규칙 확정 & 최종 합불</p>
                </div>
                <div className="p-4 rounded-xl border border-slate-200 bg-white space-y-1">
                  <p className="text-[11px] text-muted-foreground font-medium">TEAM (부서별 운영진)</p>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-xl">
                <div className="p-4 border-b border-slate-200 flex items-center justify-between flex-wrap gap-3 bg-white/[0.01]">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    >
                      <option value="ALL" className="bg-white">전체 역할</option>
                      <option value="MASTER" className="bg-white">MASTER</option>
                      <option value="SUPER" className="bg-white">SUPER</option>
                      <option value="TEAM" className="bg-white">TEAM</option>
                    </select>
                    ...
                    <select
                      ...
                      className="px-2.5 py-1 rounded-lg bg-slate-100 border border-slate-200 text-foreground text-xs outline-none cursor-pointer"
                    >
                      <option value="ALL" className="bg-white">전체 부서</option>
                      {TEAM_NAMES.map(t => (
                      ...
                      {filteredAccounts.map(acc => {
                        ...
                        return (
                          <tr key={acc.id} className="hover:bg-white/[0.015] transition-colors">
                            <td className="px-5 py-3.5 font-bold text-muted-foreground">#{acc.id}</td>
                            ...
                            {/* name */}
                            <td className="px-4 py-3.5 font-sans font-semibold text-foreground">{acc.name}</td>
                              ...
                              <span
                                ...
                                style={
                                  acc.role === "MASTER"
                                    ...
                                    : { background: "rgba(91,127,255,0.12)", color: "#8ba5ff", border: "1px solid rgba(91,127,255,0.3)" }
                                }
                              >
                                {acc.role}
                              </span>
                            ...
                            {/* term */}
                            <td className="px-3 py-3.5 text-center text-foreground font-bold">{acc.term}기</td>

                            {/* created_at */}
                            <td className="px-4 py-3.5 text-muted-foreground text-[11px]">{acc.created_at.replace("T", " ")}</td>
                            ...
                            <td className="px-5 py-3.5 text-right font-sans">
                              <div className="flex items-center justify-end gap-1.5">
                                <button
                                  onClick={() => {
                                    setPwTargetAccount({ id: acc.id, username: acc.username, name: acc.name, isSelf });
                                  ...
                                  title={isSelf ? "본인 비밀번호 변경 (현재 비번 필수)" : "MASTER 권한으로 비밀번호 강제 초기화"}
                                >
                                  <Key size={11} />
            ...
            <div className="max-w-2xl mx-auto space-y-4">
              ...
              <div className="p-4 rounded-2xl border border-blue-500/30 bg-blue-500/[0.04] flex items-start gap-3">
                ...
                <div className="text-xs space-y-1">
                  <p className="font-bold text-foreground">운영진 본인 계정 정보 및 보안 관리 (GET /api/v1/admin/accounts/me)</p>
                  <p className="text-muted-foreground">
                    타 운영진의 전체 계정 생성·수정·삭제는 <strong>서비스운영팀장(MASTER)</strong> 전용 권한입니다. 현재 로그인된 본인의 프로필 확인 및 비밀번호 변경을 안전하게 수행할 수 있습니다.
                  </p>
                </div>
              </div>
              ...
              <div className="p-6 rounded-2xl border border-slate-200 bg-white space-y-5 shadow-xl">
                <div className="flex items-center justify-between border-b border-slate-200 pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-mono text-muted-foreground mt-0.5">
                        아이디: @{currentAdmin.username} · 소속: {currentAdmin.team_name} ({currentAdmin.track})
                      </p>
                    </div>
                  </div>
                  ...
                  <button
                    onClick={() => {
                      setPwTargetAccount({ id: currentAdmin.id, username: currentAdmin.username, name: currentAdmin.name, isSelf: true });
                      setCurrentPasswordInput("");
                      setNewPasswordInput("");
                      setConfirmPasswordInput("");
                    }}
                ...
                <div className="grid grid-cols-2 gap-4 text-xs font-mono">
                  ...
                  <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-100 space-y-1">
                    ...
                    <p className="text-muted-foreground text-xs">{currentAdmin.created_at.replace("T", " ")}</p>
                  </div>
                  <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-100 space-y-1">
        ...
        <div className="space-y-4">
          <div className="p-4 rounded-2xl border border-slate-200 bg-white space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              서비스마다 role과 teamName을 직접 하드코딩하지 않고, 계정이 가진 <code className="text-foreground font-mono">Permission</code> 열거형으로 접근을 통제합니다.
            </p>
          </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50 text-muted-foreground font-semibold">
                    ...
                    <th className="text-center px-4 py-3.5 text-pink-400">MASTER (서비스운영팀장)</th>
                ...
                <tbody className="divide-y divide-white/5 font-mono">
                  {PERMISSION_MATRIX.map(pm => (
                    <tr key={pm.name} className="hover:bg-white/[0.015]">
                      ...
                      <td className="px-4 py-3.5 text-center font-sans font-bold text-pink-300">{pm.master}</td>
                      <td className="px-4 py-3.5 text-center font-sans font-bold text-purple-300">{pm.super}</td>
                      <td className="px-4 py-3.5 text-center font-sans font-bold text-blue-300">{pm.opsTeam}</td>
                      <td className="px-4 py-3.5 text-center font-sans text-muted-foreground">{pm.otherTeam}</td>
                      <td className="px-4 py-3.5 text-center font-sans text-muted-foreground">{pm.host}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <tbody className="divide-y divide-white/5 font-mono">
                  {auditLogs.map(log => (
                    <tr key={log.id} className="hover:bg-white/[0.015]">
                      <td className="px-5 py-3.5 text-muted-foreground text-[11px]">{log.timestamp}</td>
                      <td className="px-4 py-3.5 font-sans font-bold text-[#8ba5ff]">{log.actor}</td>
                      <td className="px-4 py-3.5">
                      ...
                      <td className="px-5 py-3.5 font-sans text-muted-foreground">{log.description}</td>
                      <td className="px-5 py-3.5 text-right text-muted-foreground text-[11px]">{log.ipAddress}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <form onSubmit={handleCreateAccount} className="space-y-3.5 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    <option value="TEAM" className="bg-white">TEAM (일반 운영진)</option>
                    <option value="SUPER" className="bg-white">SUPER (대표진/차기)</option>
                    <option value="MASTER" className="bg-white">MASTER (서비스운영팀장)</option>
                  </select>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    ...
                    <option value="ENGINEERING" className="bg-white">ENGINEERING (엔지니어링)</option>
                    <option value="VISUALIZATION" className="bg-white">VISUALIZATION (시각화)</option>
                  </select>
                </div>
        ...
        <div className="fixed inset-0 bg-black/85 z-80 flex items-center justify-center p-4 backdrop-blur-xs">
          <div className="w-full max-w-md rounded-2xl overflow-hidden p-6 space-y-4 bg-white border border-slate-200 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              ...
              <button onClick={() => setEditingAccount(null)} className="text-muted-foreground hover:text-foreground cursor-pointer">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="space-y-3.5 text-xs">
              <div>
                <label className="text-muted-foreground block mb-1 font-semibold">아이디 (변경 불가)</label>
                <input
                  disabled
              ...
              <div>
                <label className="text-muted-foreground block mb-1 font-semibold">성명 (name)</label>
                <input
                  value={editName}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-muted-foreground block mb-1 font-semibold">역할 (role)</label>
                  <select
                ...
                <div>
                  <label className="text-muted-foreground block mb-1 font-semibold">소속 부서 (team_name)</label>
                  <select
                    value={editTeamName}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-muted-foreground block mb-1 font-semibold">소속 트랙 (track)</label>
                  <select
                    value={editTrack}
                ...
                <div>
                  <label className="text-muted-foreground block mb-1 font-semibold">활동 기수 (term)</label>
                  <input
                    type="number"
                    min={0}
                    value={editTerm}
          ...
          <div className="w-full max-w-md rounded-2xl overflow-hidden p-6 space-y-4 bg-white border border-slate-200 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <div>
                <h3 className="text-base font-bold text-foreground flex items-center gap-2">
                  ...
                  <span>
                    {pwTargetAccount.isSelf ? "내 비밀번호 변경" : `비밀번호 강제 초기화 (${pwTargetAccount.name})`}
                  </span>
                </h3>
                <p className="text-[11px] font-mono text-muted-foreground mt-0.5">
              ...
              <button onClick={() => setPwTargetAccount(null)} className="text-muted-foreground hover:text-foreground cursor-pointer">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="space-y-3.5 text-xs">
              ...
              {pwTargetAccount.isSelf && (
                <div>
                  <label className="text-muted-foreground block mb-1 font-semibold">
                    현재 비밀번호 (current_password) <span className="text-red-400 font-bold">*</span>
                  </label>
                  <input
                    type="password"
