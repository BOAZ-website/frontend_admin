...
import {
  BookOpen, Plus, Trash2, ArrowUp, ArrowDown, Save,
  CheckCircle2, Info, Sparkles, Layers, Tag
} from "lucide-react";
...
const INITIAL_CURRICULUM: CurriculumStep[] = [
  // Analysis
  {
    ...
    description: "Pandas, NumPy를 활용한 대규모 정형 데이터 전처리 및 통계적 가설 검정과 데이터 분포 시각화 기초를 다룹니다.",
  ...
  },
  {
    ...
    description: "지도학습/비지도학습 핵심 모델(Scikit-learn, XGBoost, LightGBM) 학습 및 하이퍼파라미터 튜닝을 통한 모델 최적화 실습을 진행합니다.",
  ...
  },
  {
    ...
    description: "PyTorch 기반의 신경망 모델링, HuggingFace Transformers를 활용한 자연어 처리 및 컴퓨터 비전 실무 프로젝트를 완수합니다.",
  ...
  // Engineering
  {
    ...
    description: "RDBMS(PostgreSQL)와 NoSQL(MongoDB) 모델링, 인덱스 설계 및 대용량 쿼리 튜닝 기법을 실습합니다.",
  ...
  },
  {
    ...
    description: "Apache Spark와 Kafka를 이용한 실시간/배치 스트리밍 데이터 파이프라인 설계 및 Airflow 워크플로우 오케스트레이션을 다룹니다.",
  ...
  },
  {
    ...
    description: "Docker, Kubernetes, AWS 기반의 확장성 높은 데이터 플랫폼 인프라 구축 및 ML 서빙 파이프라인을 완성합니다.",
  ...
  // Visualization
  {
    ...
    description: "정보 디자인 원칙과 인간 인지 과정을 고려한 차트 설계, Tableau와 PowerBI를 활용한 인터랙티브 대시보드 제작을 배웁니다.",
  ...
  },
  {
    ...
    description: "D3.js, Chart.js, React 라이브러리를 결합하여 웹 브라우저 상에서 동적으로 반응하는 커스텀 시각화 컴포넌트를 구현합니다.",
  ...
  },
  {
    ...
    description: "GIS 공간 정보 시각화(Mapbox, Deck.gl) 및 대규모 사용자 인터랙션을 지원하는 최종 컨퍼런스 웹 시각화 결과물을 제작합니다.",
...
export function CurriculumSection() {
  ...
  function handleMoveStep(id: string, direction: "up" | "down") {
    ...
    setCurriculums(prev =>
      prev.map(c => {
        if (c.id === currentStep.id) return { ...c, stepNum: targetStep.stepNum };
        if (c.id === targetStep.id) return { ...c, stepNum: currentStep.stepNum };
        return c;
      })
    );
  }
          ...
          {(["ANALYSIS", "ENGINEERING", "VISUALIZATION"] as CurriculumTrack[]).map(t => {
            ...
            return (
              <button
                ...
                style={
                  selectedTrack === t
                    ? { background: "#ef4444", color: "#fff", boxShadow: "0 0 10px rgba(239,68,68,0.35)" }
                    : { color: "#64748b" }
                }
              >
                {labels[t]}
        ...
        {currentSteps.map((step, index) => (
          ...
          >
            ...
            <div className="space-y-2.5 text-xs">
              ...
              <div>
                <label className="text-[11px] text-muted-foreground block mb-1">학습 내용 및 목표</label>
                <textarea
