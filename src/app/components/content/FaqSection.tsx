...
import {
  Search, Plus, HelpCircle, ChevronDown, ChevronUp,
  Trash2, Edit3, Check, X, ArrowUpDown, Eye, EyeOff
} from "lucide-react";
...
const INITIAL_FAQ_DATA: FaqItem[] = [
  {
    ...
    answer: "네, 전공과 무관하게 데이터에 대한 열정과 기초적인 학습 의지가 있다면 누구나 지원 가능합니다. 각 트랙별 기초 지식(Python, 기초 통계 등)에 대한 이해도가 있다면 서류 및 면접에서 좋은 평가를 받을 수 있습니다.",
    orderNum: 1,
    visible: true,
  ...
  },
  {
    ...
    answer: "서류 평가는 지원 동기, 문제 해결 경험, 트랙 적합성을 종합적으로 검토합니다. 면접은 지원서 기반의 개별 인터뷰로 진행되며 실무적인 지식뿐만 아니라 협업 태도와 성장 잠재력을 중점적으로 평가합니다.",
    orderNum: 2,
    visible: true,
    updatedAt: "2026-07-20",
  },
  {
    ...
    answer: "정규 세션은 매주 토요일 오후 2시부터 6시까지 진행됩니다. 방학 중에는 온/오프라인 병행 스터디가 추가로 진행될 수 있습니다.",
    orderNum: 1,
    visible: true,
  ...
  },
  {
    ...
    answer: "출석 시 +1점, 지각 시 +0.5점, 결석 시 0점이 부여됩니다. 스터디장이 매주 출결을 입력하고 운영지원팀에서 승인하며, 총 활동 점수가 수료 기준에 반영됩니다.",
    orderNum: 2,
    visible: true,
    updatedAt: "2026-06-15",
  },
  {
    ...
    answer: "정규 활동 기간(2학기) 동안 출석률 80% 이상 및 최종 프로젝트 컨퍼런스 발표를 완료한 부원에게 정식 수료증이 발급됩니다.",
    orderNum: 1,
    visible: true,
              ...
              <div key={faq.id} className="transition-colors hover:bg-white/[0.01]">
                ...
                >
                  <div className="flex items-center gap-3 flex-1 min-w-0 pr-4">
                    <span className={`w-[68px] h-[23px] rounded-md text-[11px] font-bold border shrink-0 inline-flex items-center justify-center ${CATEGORY_STYLES[faq.category]}`}>
                  ...
                  <div className="flex items-center gap-2 shrink-0" onClick={e => e.stopPropagation()}>
                    ...
                    >
                      {faq.visible ? <Eye size={13} /> : <EyeOff size={13} className="text-red-400" />}
                    </button>
                    <button
        ...
        <div className="fixed inset-0 bg-black/75 z-50 flex items-center justify-center p-4 backdrop-blur-xs">
          <div className="w-full max-w-lg rounded-2xl overflow-hidden p-6 space-y-4 bg-white border border-slate-200 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              ...
              <button onClick={() => setEditingFaq(null)} className="text-muted-foreground hover:text-foreground cursor-pointer">
                <X size={16} />
              </button>
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setEditingFaq(prev => prev ? ({ ...prev, category: e.target.value as FaqCategory }) : null)}
                    className="w-full px-3 py-2 rounded-md outline-none bg-slate-100 border border-slate-200 text-foreground cursor-pointer"
                  >
                    <option value="RECRUITMENT" className="bg-white">리크루팅 (RECRUITMENT)</option>
                    <option value="ACTIVITY" className="bg-white">활동 안내 (ACTIVITY)</option>
                    <option value="ETC" className="bg-white">기타 (ETC)</option>
                  </select>
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setEditingFaq(prev => prev ? ({ ...prev, orderNum: Number(e.target.value) }) : null)}
              ...
              <div className="flex items-center gap-2 pt-1">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={e => setEditingFaq(prev => prev ? ({ ...prev, visible: e.target.checked }) : null)}
