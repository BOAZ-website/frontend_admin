...
import {
  Search, Plus, Image as ImageIcon, Upload, Trash2, ExternalLink,
  Check, X, ChevronLeft, ChevronRight, Eye, RefreshCw, ZoomIn,
  FolderKanban, BookOpen, Camera, CheckCircle2, AlertCircle,
  Code, FileJson, Calendar
} from "lucide-react";
...
const INITIAL_ARCHIVE_DATA: ArchiveItem[] = [
  {
    ...
    visible: true,
    links: { github: "https://github.com/boaz-analysis/delivery-churn", slideshare: "https://slideshare.net/boaz/delivery-churn" },
  },
  {
    id: "p2",
    type: "project",
  ...
  },
  {
    ...
    visible: true,
    links: { github: "https://github.com/boaz-analysis/fraud-detect", slideshare: "https://slideshare.net/boaz/fraud-detect" },
  },
  {
    id: "p5",
    type: "project",
  ...
  },
  {
    ...
    imageUrl: SAMPLE_PROJECT_IMAGES[0],
    visible: true,
    links: { github: "https://github.com/boaz-analysis/card-consumption", slideshare: "https://slideshare.net/boaz/card-consumption" },
  },
  {
    id: "p8",
    type: "project",
  ...
  // Tech Blogs
  {
    ...
    imageUrl: SAMPLE_PROJECT_IMAGES[4],
    visible: true,
    links: { medium: "https://medium.com/boaz/k8s-airflow-batch", github: "https://github.com/boaz-eng" },
  },
  {
    id: "b2",
    type: "blog",
...
export function ArchivingSection() {
  ...
  const filteredItems = items.filter(item => {
    ...
    if (selectedTerm !== "ALL" && item.term.toString() !== selectedTerm) return false;
    if (searchQuery.trim()) {
      const q = searchQuery.toLowerCase();
      return item.title.toLowerCase().includes(q) || (item.teamName ?? "").toLowerCase().includes(q);
    }
    return true;
  });
  ...
  function handleAttachSampleImage() {
    const randomImg = SAMPLE_PROJECT_IMAGES[Math.floor(Math.random() * SAMPLE_PROJECT_IMAGES.length)];
  ...
  function buildBackendPayload() {
    ...
    return {
      endpoint: isNew ? `POST ${endpointMap[activeTab]}` : `PATCH ${endpointMap[activeTab]}/${editForm.id}`,
      contentType: "multipart/form-data",
      parts: {
        data: dataPart,
        image: editForm.imageFile ? `[File: ${editForm.imageFile.name}, size: ${editForm.imageFile.size} bytes]` : (editForm.imageUrl ? `[Preserved URL: ${editForm.imageUrl}]` : "null (생략)"),
      },
    };
  }
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap pt-1">
        <div className="flex items-center gap-2.5 flex-1 min-w-[320px]">
          ...
          >
            ...
            <option value="ANALYSIS" className="bg-white">분석</option>
            <option value="ENGINEERING" className="bg-white">엔지니어링</option>
            <option value="VISUALIZATION" className="bg-white">시각화</option>
          </select>
          ...
          <select
            ...
            className="px-3 py-1.5 text-xs rounded-md outline-none bg-slate-100 border border-slate-200 text-foreground cursor-pointer"
          >
            <option value="ALL" className="bg-white">전체 기수</option>
            <option value="21" className="bg-white">21기</option>
            <option value="20" className="bg-white">20기</option>
            <option value="19" className="bg-white">19기</option>
            <option value="18" className="bg-white">18기</option>
          </select>
          ...
          <span className="text-xs text-muted-foreground font-mono">
            {filteredItems.length}건 표시 · 전체 {items.filter(i => i.type === activeTab).length}건
          </span>
        </div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  <th className="text-left px-4 py-2.5 font-semibold text-muted-foreground w-16">이미지</th>
                  <th className="text-left px-3 py-2.5 font-semibold text-muted-foreground">제목</th>
                  <th className="text-center px-3 py-2.5 font-semibold text-muted-foreground w-14">기수</th>
                  <th className="text-center px-3 py-2.5 font-semibold text-muted-foreground w-20">트랙</th>
                  <th className="text-left px-3 py-2.5 font-semibold text-muted-foreground w-24">링크</th>
                  <th className="text-center px-3 py-2.5 font-semibold text-muted-foreground w-14">노출</th>
                </tr>
              </thead>
                  ...
                  filteredItems.map(item => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-2.5">
                          <div className="w-12 h-7 rounded bg-slate-100 border border-slate-200 overflow-hidden flex items-center justify-center relative">
                            {item.imageUrl ? (
                              ...
                              <span className="text-[9px] text-muted-foreground font-mono">16:9</span>
                            )}
                          </div>
                        </td>
                        ...
                        <td className="px-3 py-2.5 text-center">
                          {item.visible ? (
                            <span className="text-[11px] text-muted-foreground font-medium">노출</span>
                          ) : (
            ...
            {editForm.imageUrl ? (
              <div className="relative rounded-lg overflow-hidden border border-slate-200 bg-slate-100 group">
                ...
                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  ...
                  <button
                    onClick={() => setEditForm(prev => ({ ...prev, imageUrl: "", imageFile: null }))}
              ...
              >
                ...
                <p className="text-xs text-muted-foreground">이미지 업로드 · 16:9 권장 (image Part)</p>
                <button
                  type="button"
                  onClick={e => {
                    e.stopPropagation();
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <div>
                ...
                <input
                  type="number"
                  value={editForm.term}
                  onChange={e => setEditForm(prev => ({ ...prev, term: Number(e.target.value) }))}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setEditForm(prev => ({ ...prev, contentDate: e.target.value }))}
            ...
            <div>
              ...
              <select
                ...
                onChange={e => setEditForm(prev => ({ ...prev, track: e.target.value as TrackType }))}
              ...
              >
                ...
                <option value="ALL" className="bg-white">ALL (공통 / 전부문)</option>
              </select>
            </div>
              ...
              <>
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setEditForm(prev => ({ ...prev, links: { ...prev.links, github: e.target.value } }))}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setEditForm(prev => ({ ...prev, links: { ...prev.links, slideshare: e.target.value } }))}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setEditForm(prev => ({ ...prev, links: { ...prev.links, web: e.target.value } }))}
                    placeholder="https://..."
                    className="w-full px-3 py-1.5 rounded-md outline-none bg-slate-100 border border-slate-200 text-foreground placeholder:text-muted-foreground/50 font-mono text-[11px]"
                  />
                </div>
              ...
              <>
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setEditForm(prev => ({ ...prev, links: { ...prev.links, medium: e.target.value } }))}
              ...
              <>
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setEditForm(prev => ({ ...prev, links: { ...prev.links, instagram: e.target.value } }))}
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-foreground">Backend API Multipart Spec Preview</h3>
              </div>
              <button onClick={() => setShowPayloadModal(false)} className="text-muted-foreground hover:text-foreground">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs font-mono">
              <div className="p-3 rounded-lg bg-slate-100 border border-slate-200">
                ...
                <p className="text-muted-foreground text-[11px]">Content-Type: multipart/form-data</p>
              </div>

              <div className="p-3 rounded-lg bg-slate-100 border border-slate-200 overflow-x-auto">
                <p className="text-xs text-amber-400 font-bold mb-2">// Part 1: data (JSON, ArchiveCreateRequest)</p>
              ...
              <div className="p-3 rounded-lg bg-slate-100 border border-slate-200">
                <p className="text-xs text-amber-400 font-bold mb-1">// Part 2: image (MultipartFile)</p>
                <p className="text-[11px] text-muted-foreground">{buildBackendPayload().parts.image}</p>
              </div>
            </div>
      ...
      {lightboxUrl && (
        ...
        >
          <div className="relative max-w-4xl max-h-[85vh] flex flex-col items-center" onClick={e => e.stopPropagation()}>
            <img src={lightboxUrl} alt="" className="max-w-full max-h-[80vh] rounded-lg shadow-2xl object-contain border border-slate-300" />
            <div className="mt-3 flex items-center justify-between w-full text-xs text-slate-700">
              <span>아카이빙 이미지 원본</span>
              <button onClick={() => setLightboxUrl(null)} className="px-3 py-1 rounded bg-white/20 hover:bg-white/30 text-white font-medium cursor-pointer">
                닫기
              </button>
            </div>
          </div>
